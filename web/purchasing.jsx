    ...
    <a href={`statement.html?m=${m.id}`} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 2 }}>خانوادهٔ {m.family} · {p.label} · تأمین‌نشده</div>
        </div>
      ...
      <div>
        ...
        <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 8 }}>
          <span className="mono">{fmt(p.remaining)}</span> تومان تا واجد شرایط وام
        </div>
      </div>
    </a>
  );
  ...
  return (
    <section>
      ...
      <p style={{ margin: '0 0 16px', fontSize: 14, color: 'var(--ink-2)', lineHeight: 1.7, maxWidth: '74ch' }}>
        {fmt(agg.count)} سهم در حال تأمین تا ارزش کامل ({fmt(agg.price)} تومان) است —
        تا سهم کامل تأمین نشود، واجد شرایط وام نیست.
